...
func main() {
	...
	go func(jogo *Jogo) {
		...
		for range ticker.C {
			...
			// Depois de mesclar/atualizar o estado local, solicita redraw seguro
			// usando termbox.Interrupt() para que o loop principal redesenhe.
			termbox.Interrupt()
		}
	}(&jogo)
	...
	for {
		...
		// Evento especial disparado por outras goroutines para forçar redraw
		if evento.Tipo == "redraw" {
			interfaceDesenharJogo(&jogo)
			continue
		}
