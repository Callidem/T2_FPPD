...
// InterfaceIniciar inicializa a interface gráfica usando termbox
...
// InterfaceFinalizar encerra o uso da interface termbox
func InterfaceFinalizar() {
	termbox.Close()
}

// InterfaceLerEventoTeclado lê um evento do teclado e o traduz para um EventoTeclado
...
// InterfaceDesenharJogo renderiza todo o estado atual do jogo na tela
...
// InterfaceLimparTela limpa a tela do terminal
func InterfaceLimparTela() {
	termbox.Clear(CorPadrao, CorPadrao)
}

// InterfaceAtualizarTela força a atualização da tela do terminal com os dados desenhados
func InterfaceAtualizarTela() {
	termbox.Flush()
}

// InterfaceDesenharElemento desenha um elemento na posição (x, y)
