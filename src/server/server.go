...
func (s *UserService) CreateUser(req *CreateUserRequest, resp *User) error {
	...
	if id, ok := s.usernameToID[req.Username]; ok {
		u := s.users[id]
		// Atualiza posição e marca ativo; também atualiza avatar/cor enviados pelo cliente
		u.PosX = req.NewPosX
		u.PosY = req.NewPosY
		u.Avatar = req.Avatar
		u.PlayerColor = req.PlayerColor
		u.Active = true
		s.users[id] = u
		*resp = u
...
func (s *UserService) UpdatePosition(req *UpdatePositionRequest, resp *UpdatePositionReply) error {
	...
	last := s.processed[req.ClientID]
	if req.Seq <= last {
		log.Printf("[RPC] UpdatePosition ignorando: ClientID=%d Seq=%d (último processado=%d)", req.ClientID, req.Seq, last)
		// Já processado: idempotente — sinaliza confirmação
		resp.OK = true
		resp.AppliedSeq = last
		return nil
	}

	// Verifica se o ClientID é válido
	u, ok := s.users[req.ClientID]
	if !ok {
		err := errors.New("usuário não encontrado")
	...
	// Aplica a atualização de posição preservando demais campos
	u.PosX = req.PosX
	u.PosY = req.PosY
	s.users[req.ClientID] = u

	// Registra seq processado apenas depois de aplicar
	s.processed[req.ClientID] = req.Seq
...
func debugDumpServerTypes() {
	...
	dump(GetUserRequest{})
	dump(User{})
	dump(ListUsersRequest{})
	dump(ListUsersReply{})
	dump(UpdatePositionRequest{})
	dump(UpdatePositionReply{})
